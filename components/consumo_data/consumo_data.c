...
esp_err_t on_client_data(esp_http_client_event_t *evt)
{
    ...
    case HTTP_EVENT_ON_DATA:
        // ESP_LOGI(TAGCONSUMODATA, "Length=%d", evt->data_len);
...
void obtener_confirmacion_dosificador(int *confirmacion)
{
    ...
    if (err == ESP_OK)
    {
        ESP_LOGI(TAGCONSUMODATA, "HTTP GET status = %d\n", esp_http_client_get_status_code(client));
    ...
    {
        ESP_LOGE(TAGCONSUMODATA, "HTTP GET request failed: %s", esp_err_to_name(err));
        *confirmacion = 0;
    }
